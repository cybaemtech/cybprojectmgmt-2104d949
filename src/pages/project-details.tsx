...
import { projectStore, teamStore, workItemStore, userStore, teamMemberStore, getLocalUser, isStoreInitialized, initStore } from "@/lib/local-store";
...
const apiRequest = async (method: string, endpoint: string, data?: any): Promise<{ ok: boolean; json: () => Promise<any>; status: number }> => {
  ...
  const teamMemberDeleteMatch = endpoint.match(/\/teams\/(\d+)\/members\/([^/]+)/);
  if (method === 'DELETE' && teamMemberDeleteMatch) {
    const teamId = parseInt(teamMemberDeleteMatch[1]);
    const userId = teamMemberDeleteMatch[2];
    teamMemberStore.remove(teamId, userId);
    return { ok: true, json: async () => ({ user_removed_from_system: false }), status: 200 };
  }
  const teamMemberPostMatch = endpoint.match(/\/teams\/(\d+)\/members/);
  if (method === 'POST' && teamMemberPostMatch) {
    const teamId = parseInt(teamMemberPostMatch[1]);
    const { userId, role } = data;
    teamMemberStore.add(teamId, userId, role);
    return { ok: true, json: async () => ({}), status: 200 };
  }
  // Default fallback
  return { ok: true, json: async () => ({}), status: 200 };
...
function canUserEditWorkItem(
  ...
  // Admin and Scrum Master can always edit anything
...
function canUserCreateType(type: string, currentUser: any): boolean {
  ...
  // Admin and Scrum Master can create anything
  if (currentUser.role === 'ADMIN' || currentUser.role === 'SCRUM_MASTER') {
    return true;
  }

  // Client Details (EPIC) and Client Requirements (FEATURE) can ONLY be created by ADMIN or SCRUM_MASTER
  if (type === 'EPIC' || type === 'FEATURE') {
    return false;
  }

  // Project Manager can create remaining types
  if ((currentUser.role as string) === 'PROJECT_MANAGER') {
    return true;
  }

  // Regular users can create STORY, TASK and BUG
...
export default function ProjectDetails() {
  ...
  // Local data - handle async store initialization cleanly to prevent reload glitch
  const currentUser = getLocalUser();
  const [isStoreReady, setIsStoreReady] = useState<boolean>(() => isStoreInitialized());
  const [project, setProject] = useState<Project | undefined>(() => projectStore.get(projectId));
  const isProjectLoading = !isStoreReady;
  const projectError = (isStoreReady && !project) ? new Error("Project not found") : null;
  const isError = isStoreReady && !project;
  ...
  const [allUsers, setAllUsers] = useState<User[]>(() => userStore.all());
  const [projectTeamMembers, setProjectTeamMembers] = useState<User[]>(() =>
    project?.teamId ? teamMemberStore.usersForTeam(project.teamId) : [currentUser]
  );

  const refetchTeamMembers = useCallback(() => {
    setAllUsers(userStore.all());
    if (project?.teamId) {
      setProjectTeamMembers(teamMemberStore.usersForTeam(project.teamId));
    } else {
      setProjectTeamMembers([currentUser]);
    }
  }, [project?.teamId, currentUser]);

  // Listen for store changes and ensure initialisation finishes smoothly
  useEffect(() => {
    const handleStoreChange = () => {
      setIsStoreReady(isStoreInitialized());
    ...
    return () => window.removeEventListener("store-change", handleStoreChange);
  }, [projectId, refetchTeamMembers]);
  ...
  const saveInlineEdit = async (itemId: number, field: EditableField) => {
    ...
    if (field === 'assignee') {
      // Handle assignee field specially - convert to string or null
      updateData.assigneeId = value && value !== 'unassigned' ? value : null;
    } else if (field === 'actualHours') {
  ...
  // Show loading state while store initializes asynchronously (prevents reload glitch)
  if (isProjectLoading || !isStoreReady) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50/50">
        <div className="text-center p-6 bg-white rounded-xl shadow-sm border border-slate-100 max-w-sm">
          <div className="animate-spin rounded-full h-10 w-10 border-3 border-indigo-600 border-t-transparent mx-auto mb-4"></div>
          <h3 className="text-base font-semibold text-slate-800 mb-1">Loading project details...</h3>
          <p className="text-xs text-slate-500">Synchronizing workspace data</p>
        </div>
      </div>
    );
  }
  ...
  // Show error state ONLY after store has finished initializing
  if (isStoreReady && (projectError || isError || !project)) {
    return (
      <div className="flex items-center justify-center min-h-screen">
                      ...
                      <div>
                        ...
                        <p className="text-sm">
                          {project.clientAccountManager
                            ? allUsers?.find(u => String(u.id) === String(project.clientAccountManager))?.fullName || 'Unknown'
                            : 'Unassigned'}
                        </p>
                      </div>
                      <div>
                            ...
                            <div className="flex-1 min-w-0">
                              {editingCell?.itemId === item.id && editingCell?.field === 'title' ? (
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  {/* Delete button for TASK and BUG items - Admin/Scrum Master only */}
                                  {['TASK', 'BUG'].includes(item.type) && currentUser && (currentUser.role === 'ADMIN' || currentUser.role === 'SCRUM_MASTER') && (
                                    <div className="flex gap-1 ml-auto">
                                      <button
                                        onClick={(e) => {
                                          e.stopPropagation();
                          ...
                          {editingCell?.itemId === item.id && editingCell?.field === 'assignee' ? (
                            ...
                            >
                              ...
                              <SelectContent>
                                ...
                                {(() => {
                                  const userList = currentUser?.role === 'ADMIN' ? (allUsers || []) : (projectTeamMembers || []);
                                  return userList.length > 0 ? userList
                                    .filter(user => {
                                      if (['TASK', 'BUG'].includes(item.type) && currentUser?.role !== 'ADMIN' && currentUser) {
                            ...
                            >
                              <div className="w-4 h-4 rounded-full bg-blue-100 flex items-center justify-center text-[9px] text-blue-700 mr-1 flex-shrink-0">
                                {(Array.isArray(allUsers) ? allUsers.find(u => String(u.id) === String(item.assigneeId))?.fullName?.substring(0, 1)?.toUpperCase() : null) || "?"}
                              </div>
                              <span className="text-[10px] text-gray-700 truncate max-w-20">
                                {(Array.isArray(allUsers) ? allUsers.find(u => String(u.id) === String(item.assigneeId))?.email : null) || "Unknown"}
                              </span>
                            </div>
                          ) : (
                            <span
                        ...
                        {filteredDocs.map((doc) => (
                          <tr key={doc.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <div className={`w-8 h-8 rounded flex items-center justify-center text-xs font-bold ${doc.type === 'PDF'
                                  ...
                                  ?.map(user => (
                                    <SelectItem key={user.id} value={user.id.toString()}>
                                      <div className="flex items-center space-x-2">
                                        ...
                                        <span>{user.email || user.fullName || user.username}</span>
                                      </div>
                                    </SelectItem>
                                  )) || []}
                              </SelectContent>
