...
import { projectStore, teamStore, workItemStore, projectMemberStore, teamMemberStore, getLocalUser } from "@/lib/local-store";
...
export default function Projects() {
  ...
  const users: User[] = [currentUser as unknown as User].filter(Boolean);
  ...
  React.useEffect(() => {
    window.addEventListener("store-change", refetchProjects);
    // Initial fetch in case store loaded before component mounted
    refetchProjects();
    return () => window.removeEventListener("store-change", refetchProjects);
  }, []);
  ...
  const filteredProjects = useMemo(() => {
    let result = projects.filter((project: Project) => {
      ...
      if (statusFilter === "dailyStandup") {
        ...
        const isTeamMember = currentUser && project.teamId && teamMemberStore.byTeam(project.teamId).some(m => String(m.userId) === String(currentUser.id));
        return isTeamMember;
      }
      ...
      const isIndividualMember = currentUser && projectMemberStore.isMember(project.id, String(currentUser.id));
      if (isIndividualMember) return true;
      ...
      // Project creator
      if (currentUser && String(project.createdBy) === String(currentUser.id)) return true;
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredProjects.map((project: Project) => {
                    ...
                    const teamMemberIds = project.teamId ? teamMemberStore.byTeam(project.teamId).map(m => m.userId) : [];
